...
| `census-pct.py [asm] [docsDir]` | Percentage view of the coverage census: narrated / catalogued / classified / unaccounted fractions of reached game types, plus the whole-assembly reached-type/method fractions. Runs `Coverage.exe` + `Census.exe` live (Python 3, no build step). |
