...
CONST_ALLOWLIST = {
    ...
    "PrefabPreviewManager",
    "ReflectionManager",
    "SignalProcessing",
    "SleeperVolumeToolManager",
    "StreetTile",
...
def main() -> int:
    ...
    for cls, (doc_name, consts) in CONSTS.items():
        ...
        for name, want in consts.items():
            ...
            grouped = (
                name.endswith("Sq")
                or name.startswith("cBlockerFlag")
                or name.startswith("BlockFaceDrawn")
                or name.startswith("BT_")
                or name.startswith("cPath")
                or name.endswith("Max")
                or name.startswith("cWalkType")
                or name.startswith("cSync")
                or name.startswith("ChunkBlock")
                or name.startswith("HM_")
            )
    ...
    for fam in cout.splitlines():
        ...
        if fam and fam not in pinned_fams and fam not in CONST_ALLOWLIST:
            bad.append(
                f"const-rich class `{fam}` is neither pinned nor allowlisted (pin it or add to CONST_ALLOWLIST)"
            )
    if bad:
        for b in bad[:25]:
            print("FAIL:", b)
