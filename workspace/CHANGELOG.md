...
## 2026-08-11 - tier-C: 7dtd-realworld CI added. .github/workflows/ci.yml gates `make test-fast` (pure-python coordinate/tile/height/height-mod-case tests, numpy only) - the first CI for the product repo; make check itself needs the game install (setup/build). Green on first run. Every CI-able repo in the fleet now has a green GitHub Actions gate (research, loadgen, optimizer, apm, server-guard, realworld; zdtd has its own; connect/playtest builds reference game DLLs).
...
## 2026-08-11 - tier-C: research CI added + green. .github/workflows/ci.yml on 7dtd-research runs `make test-docs` (transport-closure claims, coverage-consistency, doc-link+hub integrity) - the DLL/mono/il-free doc gates. Two CI iterations needed: test_dedi_coverage_docs.py reads git-ignored il/ dumps + the realworld sibling, so it stays in the local make test (also made sibling-tolerant). Full fleet green on GitHub Actions: research, optimizer, server-guard, loadgen, apm. connect/playtest/realworld have no CI (builds reference game DLLs; not feasible). ywy50 collaborator (push) confirmed on all 9 repos.
## 2026-08-11 - tier-C: CI green across the fleet. Four repos audited on GitHub Actions: 7dtd-optimizer (no CI -> added, success), 7dtd-server-guard (added; AGENTS.md ../sibling links broke doccheck in a single-repo checkout -> doccheck now skips ../ links like the research link test, success), 7dtd-loadgen (RealEarth sibling tests failed in CI -> skipped when REALEARTH_ROOT absent, success; selftest folded into make test so CI covers the join path), 7dtd-apm (check-bt cannot resolve bpftrace probes in a containerized runner -> new check-ci target for CI, success). All 4 now green; local make check/test still run the full gates including check-bt + sibling-dependent tests.
## 2026-08-11 - tier-C: sibling gate sweep. Three latent red gates fixed: 7dtd-loadgen RealEarth scenario port test asserted the serverconfig ServerPort against loadgen's client data port (which is ServerPort+2 per run_scenario.sh; the JSON was right, the test had failed since its 2026-08-09 rename) - now parses ServerPort and checks defaults + live scenarios against ServerPort+2; 7dtd-apm make check was red on ruff format-check (3 files) and mypy (scrub test indexed an object return; whole-structure equality instead); 7dtd-optimizer make test failed on hosts without dotnet on PATH - added the cached-SDK locator from loadgen's Makefile. CI added where missing: optimizer + server-guard (both previously unguarded; server-guard had a ci target with no workflow). optimizer/loadgen/apm/server-guard/zdtd gates all green now. zdtd fuzz.zig bool->u8 fix was already committed by a concurrent session (c2e109d); repo has active concurrent worktrees - left untouched.
