...
## 2026-08-12 - tier-C: zdtd citation verifier + provenance gate. New tools/zdtd_cite_check.py (make zdtd-cites): extracts every EXPLICIT research citation from the zdtd repo (RE: markers, 7dtd-research/docs paths, and bare doc names in src comments resolved against research docs + zdtd-local/ADR names) and verifies the cited doc exists - 169 citations all resolve. Caught 2 real typos in zdtd (scenarios.zig "weather-env.md" -> weather-environment.md; c2s/misc.zig "netpackages.md" -> protocol-packages.md) plus the pass-2 sleeper cap-bypass divergence and the honest party-XP row (shared reduction not yet implemented). All gates green.
...
## 2026-08-12 - tier-C: provenance-review prompt moved to zdtd. docs/provenance-review-prompt.md removed from research; the reusable prompt now lives at ../../zdtd/docs/PROVENANCE_REVIEW_PROMPT.md (with the zdtd hub). Research INDEX row repointed; coverage audit row dropped. All gates green; pushed.
## 2026-08-12 - tier-C: provenance-review prompt renamed for ~/review-prompts discovery. zdtd/docs/provenance-review.md (was PROVENANCE_REVIEW_PROMPT.md) - the name ends in -review.md so the review-loop tool picks it up when run with zdtd as cwd. Research INDEX link updated. All gates green; pushed.
