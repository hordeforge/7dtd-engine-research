...
The exact 3.1.0→3.2.0 delta is [`changelog-3.2.0.md`](../releases/changelog-3.2.0.md).

## Feature inventory

| Feature (official) | RE home / implementation note | Status |
|---|---|---|
| **Chicken farming**: sneak up on wild chickens, grab/carry them, defend them, place in a Chicken Coop | Held-entity grab is the `ItemClassHeldEntity` family + `EntityAlive.InitLocalActivationCommands`/`OnEntityActivated("grab")` ([items.md](../gameplay/items.md) § Held entities). Implement: entity spawn/despawn with a holder, `NetPackagePlayerId`-style sync for the held slot. | covered |
| **Chicken Coop workstation** with 3 tool slots: Brooding Lamp (production/maturation speed), Chicken Run (-50% feed), Nesting Box (breeding + nesting slots) | Composite TE with slots; the coop knobs map to sandbox options ([tile-entities-power.md](../gameplay/tile-entities-power.md), [sandbox-options.md](../admin/sandbox-options.md) § Crafting). Implement: `TEFeature` slot inventory + per-option production tick. | covered |
| **Chicken Stress event** (stress to 100% → drop chicken + stage event; sandbox-tunable) | Sandbox option `ChickenStressEvent` + `FullChickenStressEvent` ([sandbox-options.md](../admin/sandbox-options.md) §3); client FX via game events. Implement: stress accumulation on the held-entity side, event dispatch via `GameEventManager`. | covered |
...
| **Discord**: DM-notification audible option, cancel-login/unlink buttons | client UI + platform SDK; dedi surface is only `DiscordManager` (minimal; see [dedicated-misc-systems.md](../meta/dedicated-misc-systems.md)). | content / dedi-minimal |
...
|---|---|---|
| "Player inventory data could be sent to server when nothing had changed" | The inventory transaction **hash guard**: `NetPackageInventoryDataRequest/Response` keyHash+Guid; body `InventoryTransaction.Write` ([items.md](../gameplay/items.md) § Inventory transactions). Implement: hash the container state, skip unchanged sends. | covered |
| "Celebration Sandbox Option no longer blocks shared XP" | Party-share XP path: `GameManager.SharedKillServer` / `EntityAlive.PartyShareKillServer` (3.2.0 rework; [combat-damage.md](../gameplay/combat-damage.md) §3.1a). Implement: only the killer's own XP is scaled by celebration; share is not. | covered |
| "EOS exception in TileEntityNetPackage during POI reset" | EOS guard in the TE-net/POI-reset path; present in the dump, no dedicated prose. | backlog ([residuals.md](../meta/residuals.md) §1) |
...
| Topic | Home |
|---|---|
| NetPackageTileEntity `teBlockId` + i32 length | [protocol-packages.md](../network/protocol-packages.md) §6.12, [tile-entities-power.md](../gameplay/tile-entities-power.md) |
| Held entities / wild chicken grab | [items.md](../gameplay/items.md) § Held entities, [entity-ai.md](../entities/entity-ai.md) |
| WorldState.SaveLoad IL=926 / CurrentSaveVersion=23 | [save-region.md](../world/save-region.md) §1 (full-file byte-exact round-trip; game-reader round-trip; shipped V4.0-tooled world skew) |
...
| Join analytics `PlayerJoinServerEventData` | [server-lifecycle.md](../admin/server-lifecycle.md) |
| Sandbox day/night density+respawn, chicken coop knobs, six difficulty preset codes, infection/hunger/thirst/stack | [sandbox-options.md](../admin/sandbox-options.md) §2 + §3 |
| EOS/browse filters, GSI sandbox fields | [server-browser-prefabs.md](../admin/server-browser-prefabs.md), [network.md](../network/network.md) |
| Official product notes (content, not IL) | https://7daystodie.com/v3-1-0-henpocalypse-release-notes/ |

## Related

- Exact delta to 3.2.0: [`changelog-3.2.0.md`](../releases/changelog-3.2.0.md) §5a.
- V3.0.0 (Dead Hot Summer) feature inventory: [`changelog-3.0.0.md`](../releases/changelog-3.0.0.md).
