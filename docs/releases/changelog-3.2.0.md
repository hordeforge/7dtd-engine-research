...
**To (sections 1–7):** V3.2.0 b9 (Major=3 Minor=20 Build=9), dated 2026-08-28.
**Current pin:** V3.2.0 b10 (Major=3 Minor=20 Build=10). The b9→b10 patch is §8.
...
**Date:** 2026-08-28 (body); §8 added 2026-09-05. All IL line counts are exact from the dumps.
...
A V3.1.0 peer cannot parse a V3.2.0 body. Docs: [protocol.md](../network/protocol.md) §6.5,
[protocol-packages.md](../network/protocol-packages.md) §6.11.
...
- Docs: [protocol-packages.md](../network/protocol-packages.md) § POI metadata.
...
- Docs: [protocol-packages.md](../network/protocol-packages.md) §5.1/§5.1.2,
...
- Docs: [items.md](../gameplay/items.md) §2 row 11, §7.1.
...
- Docs: [tile-entities-power.md](../gameplay/tile-entities-power.md),
...
- Docs: [items.md](../gameplay/items.md) §7.1.
...
- Docs: [combat-damage.md](../gameplay/combat-damage.md) §3.1a.
...
- Docs: [entity-ai.md](../entities/entity-ai.md) § EAIRunawayFromEntity.
...
- Docs: [world-generation.md](../world/world-generation.md), [spawning.md](../gameplay/spawning.md).
...
- Docs: [quests-challenges.md](../content/quests-challenges.md).
...
RE/implementation pointers: [`changelog-3.1.0.md`](../releases/changelog-3.1.0.md).
Managed-relevant items and their corpus home:

| V3.1.0 official item | Status |
|---|---|
| Chicken farming: catch/carry (held entities), coop workstation, stress events | **covered** — [items.md](../gameplay/items.md) § Held entities, [tile-entities-power.md](../gameplay/tile-entities-power.md), [sandbox-options.md](../admin/sandbox-options.md) |
| New sandbox options (ChickenCoopTime/Output/Input, ChickenStressEvent, InfectionChance, HungerMultiplier, ThirstMultiplier, StackSizeMultiplier) + day/night + enemy/animal density/respawn split | **covered** — [sandbox-options.md](../admin/sandbox-options.md) §2-3, INDEX delta map |
| Diamond Blade Tip mod in trader/loot lists | content (XML) |
| Discord DM-audible / cancel-login / unlink | client UI + platform SDK; dedi surface only via `DiscordManager` (minimal, see [dedicated-misc-systems.md](../meta/dedicated-misc-systems.md)) |
| Fix: "Player inventory data could be sent to server when nothing had changed" | **covered** — the inventory transaction hash guard ([items.md](../gameplay/items.md) § Inventory transactions; `NetPackageInventoryDataRequest/Response` keyHash+Guid, protocol-packages.md) |
...
## 8. V3.2.0 b9 → b10 patch (2026-09-05)

**Steam / install:** dedicated app 294420 buildid `24994542` (client 251570
buildid `24994517`), both downloaded 2026-09-05 ~08:35 UTC. SteamDB history
returned HTTP 403 from this environment, so the delta below is from a local
pairwise dump of the live `Assembly-CSharp.dll` against the retained b9
backup (`Assembly-CSharp.dll.re_stock_bak`, sha256
`70c669bec3cb11d1d8fabd2cc4f871d1eca21e27d3a185afe3efa71e571cabd6` → live
`3737eedc9f143d428a69030317428c45d342be4f11006a46d72bcd7b1d0fc64d`).

**Pin only (machine):** `Constants.cVersionBuild` 9 → 10;
`version.stock_wire` `V3.2.0 b9` → `V3.2.0 b10`. Census unchanged:
4426 top-level types / 44277 methods-with-body / 7451 all types / 195
NetPackage / `gmUpdate` IL=631 / `WorldState.SaveLoad` IL=926.
`xml_pins.json` byte-identical to the b9 extract.

**Managed code (verified IL):** only the EOS Title Storage client path
`Platform.EOS.RemoteFileStorage` (+ nested `RequestDetails`) changed.
Surface metadata: `RequestDetails` fields 4 → 6 (+`Transfer`,
+`Cancelled`); one Burst job-reflection type rename
(`__JobReflectionRegistrationOutput__2130389565` →
`__JobReflectionRegistrationOutput__1706723748`, same IL=13). No
NetPackage / wire / sim-loop / save-format drift.

| Method | b9 IL | b10 IL | Change |
|---|---:|---:|---|
| `CancelGetFile(String,String)` | 16 | 70 | Under `requestsLock`, sets `Cancelled`; if `Transfer != null`, calls `TitleStorageFileTransferRequest.CancelRequest()` under `AntiCheatCommon.LockObject` (warns unless EOS `Result` is Success=`0` or NoChange=`20`; `AlreadyPending` is `9`); then `CompleteRequest(..., EFileDownloadResult.Other, cancelReason, null)` (`ldc.i4.3`; enum fields Ok=0, EmptyFilename=1, FileNotFound=2, Other=3) |
| `TryProcessNextRequest` | 38 | 41 | Skips queue entries with `Cancelled` before incrementing `activeRequests` / `getMetadata` |
| `CompleteRequest` | 41 | 73 | `TryGetValue` + reference-eq on the tracked entry; on miss/mismatch logs `[EOS] Ignoring completion of RFS request no longer tracked: …` via `Log.Out` and returns (b9 used `Remove` + `Log.Warning` “Unexpected RFS request…”); on hit: Remove, decrement `activeRequests`, `TryProcessNextRequest`, then `ExecuteCallback` |
| `readFile` | 44 | 45 | Stores the EOS `TitleStorageFileTransferRequest` from `TitleStorageInterface.ReadFile` into `RequestDetails.Transfer` |
| `queryFileCallback` | 80 | 83 | Early-out when `Cancelled` (before metadata copy / `readFile`) |
| `readCompletedCallback` | 165 | 169 | Early-out when `Cancelled` |
| `getMetadata` / `readFileDataCallback` / Local `CancelGetFile` | unchanged | unchanged | Local stub still `NotImplementedException` |

**Body-hash completeness (verified):** every method body in b9 bak vs live
b10 hashed (`tools/asm_body_diff.py`): 56064 methods; 2 added / 2 removed
(Burst `__JobReflectionRegistrationOutput__*` rename only); **6 body
...
`traders`, `loot`, `entityclasses`, `progression`, `vehicles`,
`gameevents`, Localization, XUi). `Unity.Collections.dll` also refreshed
...
family changed; method dumps under `.scratch/b10-il/` for review).

---

## 9. Per-fact homes (delta map)
...
[`changelog-3.2.0.md`](../releases/changelog-3.2.0.md). The current pin is **b10**; the
b9→b10 patch (EOS Title Storage cancel path only) is §8 of that changelog and
[`platform-auth.md`](../admin/platform-auth.md) §6. The per-fact homes for the major
3.1.0→3.2.0 delta:

| Topic | Home |
|---|---|
| Machine pin (version, TPS, census, TE widths) | [`../../tools/data/stock_facts.json`](../../tools/data/stock_facts.json), [coverage.md](../meta/coverage.md), [re-methodology.md](../meta/re-methodology.md) §1 |
| Wire: `NetPackageDamageEntity` packed `UInt32 flags` bitfield + `KillXPScale` (drops 10 bools, drops `bIsDamageTransfer`) | [protocol.md](../network/protocol.md) §6.5, [protocol-packages.md](../network/protocol-packages.md) §6.11 |
| Wire: `NetPackagePOIAround` removed → `NetPackagePOIMetadataRequest/Response` + `PrefabInstance.POIMetadata` (server pushes POI metadata to clients) | [protocol-packages.md](../network/protocol-packages.md) § POI metadata, [protocol.md](../network/protocol.md) §6 |
| Wire: `NetPackageConfirmSpawnEntity` (S2C) + `EntityCreationData.requestedBy`/`requestKey` (spawn request correlation) | [protocol-packages.md](../network/protocol-packages.md) § ConfirmSpawnEntity, [spawning.md](../gameplay/spawning.md) |
| Kill XP rework: `DamageSource.bTrapKillXP`/`KillXPScale`, `EntityAlive.AwardKillXPServer`/`PartyShareKillServer`, `EntityPlayer.AddKillXP` | [combat-damage.md](../gameplay/combat-damage.md), [entity-stats.md](../entities/entity-stats.md) |
| Trader doors: `TEFeatureDoor.HonkOpenType`/`HonkOpenDistance`, `TraderDoorController` (new), `Vehicle.GetHornEventName` | [tile-entities-power.md](../gameplay/tile-entities-power.md), [vehicles-drones-turrets.md](../gameplay/vehicles-drones-turrets.md) |
| Item combining: `ItemValue.Flags` + `cFlagsWasCombined`, `ItemActionEntryCombine` (new), `XUiC_CombineGrid` rework | [items.md](../gameplay/items.md) § Combining |
| Decoration suppression: `DecoSuppressArea` (new), `DynamicPrefabDecorator` deco-suppress store, `AllowDecorations` prop, `AllowTopSoilDecorations` removed | [world-generation.md](../world/world-generation.md), [spawning.md](../gameplay/spawning.md) |
| AI: `EntityFlags.Timid = 32` (new), `EAIRunawayFromEntity` flag-based threat + `dangerDistance` | [entity-ai.md](../entities/entity-ai.md) |
| Misc: `Challenge.CompleteChallenge(..., forceComplete)`, sandbox option display API (`GetDisplayAtIndex(index, languageName)`), analytics sandbox-delta events, `StockFileHashes` all regenerated | [quests-challenges.md](../content/quests-challenges.md), [sandbox-options.md](../admin/sandbox-options.md) |

Prior-release feature inventories (official changelogs mapped to RE homes):
[`changelog-3.1.0.md`](../releases/changelog-3.1.0.md) (Henpocalypse) ·
[`changelog-3.0.0.md`](../releases/changelog-3.0.0.md) (Dead Hot Summer).
