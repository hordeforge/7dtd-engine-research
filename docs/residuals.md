...
| **Encryption cipher / KDF primitives** | Handshake package bodies now decoded (`EncryptionRequest -> PublicKey -> SharedKey -> KeyExchangeComplete`, [protocol-packages.md](protocol-packages.md) §2); only the underlying cipher/KDF calls are native (below managed) |
