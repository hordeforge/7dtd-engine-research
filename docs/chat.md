...
A client sends a chat `NetPackageChat`; `ChatMessageServer` logs it, runs the
`ModEvents.ChatMessage` hook (mods may rewrite or suppress, "Chat handled by mod"),
then fans it out. The routing decision is **recipient-list based, not channel
based**: if the package carries a non-empty `recipientEntityIds`, the server sends
only to those clients; otherwise it broadcasts to all. The **`EChatType` value is
carried through** for the client to display the channel, but the server does **not**
itself resolve party/friends membership: the sender (client) supplies the recipient
id list for a party/whisper message.
