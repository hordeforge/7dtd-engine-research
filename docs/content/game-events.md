...
the main server loop ([`loop-gmupdate.md`](../loop/loop-gmupdate.md), manager chain).
...
([`loop-gmupdate.md`](../loop/loop-gmupdate.md), step 5;
[`managers.md`](../loop/managers.md)) and fans into five bookkeeping passes plus the
sequence tick of §2:
...
ChangeBlocks machine, [`blocks.md`](../world/blocks.md) §4.1). Once the list empties,
...
- **Idle cost is near zero:** with no running sequences the five passes iterate
  empty lists ([`entity-ai.md`](../entities/entity-ai.md) records the 25-IL `Update` shell
...
| Doc | Role |
|---|---|
| [`full-surface.md`](../meta/full-surface.md) | Where `GameEvent.*` sits in the whole-assembly map |
| [`loop-gmupdate.md`](../loop/loop-gmupdate.md) | The manager chain that ticks `GameEventManager.Update` |
| [`managers.md`](../loop/managers.md) | Sibling in-process managers |
| [`protocol.md`](../network/protocol.md) | Wire framing; `NetPackageGameEventRequest`/`Response` context |
| [`entity-ai.md`](../entities/entity-ai.md) | Spawned-entity behavior once an event spawns something |
