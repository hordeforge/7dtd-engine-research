...
(`GamePath.PathFinderThread` / ASP, owned by [`entity-ai.md`](../entities/entity-ai.md));
drone persistence, streaming, and sync (owned by
[`vehicles-drones-turrets.md`](../gameplay/vehicles-drones-turrets.md)); the Aron Granberg
A* library internals (third-party residual, see
[`closed-gaps.md`](../entities/closed-gaps.md)).
...
`GamePath.PathFinderThread` (ASP coroutine driver, [`entity-ai.md`](../entities/entity-ai.md)
...
then `Block.intersectRayWithBlock` (see [block-shapes.md](../world/block-shapes.md)
...
- `RaycastPathManager`: `Init` in the `GameManager.StartAsServer` coroutine,
  `Update` from `gmUpdate` ([`loop-gmupdate.md`](../loop/loop-gmupdate.md) peer list).
...
1. **No `currentPath` yet** → `GetPath`. Start and end are projected to the
   ...
   [`entity-ai.md`](../entities/entity-ai.md) §2). Returned `PathPoint.projectedLocation`
...
reading is retracted ([residuals.md](../meta/residuals.md)). The search algorithm the
...
|---|---|---|
| State machine + steering + `move` (`OnUpdateEntity`/`updateTasks` in the entity tick) | yes (authority; drone is server-simulated, [`vehicles-drones-turrets.md`](../gameplay/vehicles-drones-turrets.md)) | remote copy interpolates |
...
- The steering fallback needs no path data at all: it degrades to Seek/Flee
  ...
  [`entity-ai.md`](../entities/entity-ai.md) §3).
...
| Doc | Role |
|---|---|
| [entity-ai.md](../entities/entity-ai.md) | The production A* (`PathFinderThread`/ASP) this system hands off to |
| [vehicles-drones-turrets.md](../gameplay/vehicles-drones-turrets.md) | Drone manager, streaming, teleport-to-owner, sync |
| [closed-gaps.md](../entities/closed-gaps.md) | ASP→A* residual status; third-party Granberg internals |
| [loop-gmupdate.md](../loop/loop-gmupdate.md) | `gmUpdate` peer list including the `RaycastPathManager` tick |
| [full-surface.md](../meta/full-surface.md) | Namespace coverage row for `RaycastPathing` |
| [console-commands.md](../admin/console-commands.md) | Console command surface (the `jd`/`jds` drone debug command lives in `ConsoleCmdJunkDrone`) |
