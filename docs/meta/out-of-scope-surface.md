...
**Narrated twin:** [`client-side-surface.md`](../meta/client-side-surface.md) gives each
...
`tools/src/RefScan` pass. The promotion list is a committed tool input,
[`../../../tools/data/promoted-types.txt`](../../../tools/data/promoted-types.txt):
any regeneration of this doc must exclude those names, and
`tools/tests/test_promoted_types.py` fails if one re-enters. A naive re-run of
the name classifier would silently pull them back in, so treat this file as
hand-maintained and re-verify with RefScan (referrer, not name) before changing
any classification.  
...
longer merely catalogued: [`sdcs-character-gear.md`](../content/sdcs-character-gear.md)
...
Twitch viewer/point/subscription data + event clients. Server slice is in [twitch-integration.md](../social/twitch-integration.md).
...
Steam/EOS user-data, remote/title storage, app wrappers. Identity slice is in [platform-auth.md](../admin/platform-auth.md).
...
`SimplexNoise` (see [dedicated-leftovers.md](../meta/dedicated-leftovers.md)).
...
**skipped on dedicated** ([server-lifecycle.md](../admin/server-lifecycle.md) § Join analytics).
...
| Doc | Role |
|---|---|
| [client-side-surface.md](../meta/client-side-surface.md) | Narrated twin (role lines for the census) |
| [dedicated-leftovers.md](../meta/dedicated-leftovers.md) | Server-side leaves promoted out of this list |
| [coverage.md](../meta/coverage.md) | Census and how the tiers are counted |
