...
**Not:** RealEarth product attack plan (`product ENGINE_LIMITATIONS`), optim backlog ([OPTIMIZATION_CANDIDATES](../../../7dtd-server-optimizer/docs/OPTIMIZATION_CANDIDATES.md)), permanent non-IL residuals only ([residuals.md](../meta/residuals.md)).  
**Hub:** [INDEX.md](../INDEX.md).  
**Live scale numbers:** [measured-scaling.md](../../../7dtd-server-optimizer/docs/measured-scaling.md).  
**Loop map:** [loop.md](../loop/loop.md).
...
|---|---|---|---|---|
| **Single-thread-dominated sim** | [loop.md](../loop/loop.md), [ARCHITECTURE](../../../7dtd-server-optimizer/docs/ARCHITECTURE.md) | Extra cores do not parallelize `gmUpdate` / `UpdateTick` | **Hard** | Starve work (LOD, caps); host pin one L3 ([HOST_TUNING](../../../7dtd-server-optimizer/docs/HOST_TUNING.md)); do not invent full MT sim in a Harmony mod |
| **Target ~20 game ticks/s** | [closed-gaps.md](../entities/closed-gaps.md) `GameTimer(20)` | ~50 ms budget per tick under load | **Hard** | Measure tick p99; reduce sim volume before chasing topology |
...
| **Peer script order** | **Closed 2026-08-09 (runtime)** | SdtdConsole -> ConnectionManager.Update -> GameManager.Update -> (WorldEnvironment/DynamicMeshManager) -> CM.LateUpdate -> GM.LateUpdate; CM always precedes GM (518 frames, [loop.md](../loop/loop.md) §1.1) | **Closed** | Treat as same-phase peers |
| **Dual entity paths** | [loop.md](../loop/loop.md) §3, [entity-ai.md](../entities/entity-ai.md) | Authority is `World.TickEntity`; Unity `Entity.Update` may still run if GO enabled | **Hard** | Optim must not assume one path; measure dual cost |
...
| **Entities observer-gated** | [measured-scaling.md](../../../7dtd-server-optimizer/docs/measured-scaling.md) §2 | Zero players → zombies exist but barely tick | **Soft** | Load tests need observer bots; empty server is not AI cost |
...
Live APM + loadgen ladders (2026-07-17/18). Detail: [measured-scaling.md](../../../7dtd-server-optimizer/docs/measured-scaling.md).
...
| **Registry caps** | stock constants (machine-pinned): `ChunkManager.cMaxChunksSupported` **100000**, `ChunkManager.cMaxChunksAroundPlayers` **15**, `VehicleManager.cMaxVehicles` **500**, `DroneManager.cMaxDrones` **500**, `TurretTracker.cMaxTurrets` **500** (all RE-pinned in `test_tuned_constants.py`); `SleeperVolume` spawn points **255** per volume (inline literal, IL-pinned in [`spawning.md`](../gameplay/spawning.md)) | hard registries: hit the cap and creation fails | **Soft** | stay under the caps; a clone must match or raise them |
...
| Limit (stock) | Evidence | Why it matters | Severity | What you can do |
|---|---|---|---|---|
| **LiteNetLib + managed wrappers** | [network.md](../network/network.md) | Protocol pump on main peer Update | **Hard** | Measure package cost; do not reimplement combat net |
| **LiteNetLib event dispatch (managed)** | **Closed 2026-08-10** | `UnsyncedEvents=true`; receive-thread `ConnectionRequestCheck` races `Clients.List` under join churn ([network.md](../network/network.md) §4.0) | **Closed** | Ramp bot joins (`--ramp-ms`); fix direction documented |
...
| **World mutation is main-thread culture** | [SIM_PARALLELISM](../../../7dtd-server-optimizer/docs/SIM_PARALLELISM.md) | Random `Task.Run` into EntityAlive races | **Hard** | Starve work; do not evacuate stock sim |
...
| Limit (stock) | Evidence | Why it matters | Severity | What you can do |
|---|---|---|---|---|
| **`ChunkBlockYDim = 256`** | [terrain-height.md](../world/terrain-height.md) | ~0-255 playable columns | **Hard** | RealEarth expand **or** accept compress / short peaks |
...
| **Chunk write/read layer loop `i < 64`** | [save-region.md](../world/save-region.md) IL hardcoded | Expand must rewrite save/load loops, not only WorldConstants | **Hard** | Patcher sites + soak |
| **`World.toBlockY` = `y & 255`** | light-mesh-water / surfaces | Mask clips tall Y | **Hard** | Expand Y-bound methods |
| **Light / stability / mesh start at 255/256** | [light-mesh-water.md](../world/light-mesh-water.md) | Tall columns wrong sun/mesh without expand | **Hard** | Expand checklist (`realearth-surfaces` §7.1) |
...
| **ItemStack.Clone fan-out** | [items.md](../gameplay/items.md) Clone triage | 162 sites; ~56 client UI; dedi mass is TE + inventory + few net Setups | **Soft** (alloc) | Do not Harmony XUi for dedi STW; any share-not-clone needs identity proof |
...
| Limit (stock) | Evidence | Why it matters | Severity | What you can do |
|---|---|---|---|---|
| **Boehm Mono GC** (conservative, non-generational STW) | [runtime-tuning.md](../../../7dtd-server-optimizer/docs/runtime-tuning.md) | Cannot swap collector under stock Unity | **Hard** | Cut alloc; optional incremental env (EAC-safe); measure STW |
| **Forced `GC.Collect` ~every 120 s** in gmUpdate | [runtime-tuning](../../../7dtd-server-optimizer/docs/runtime-tuning.md), A7 | Self-inflicted hitch under moderate load | **Soft** | EfficientServer GcGuardPatch (skip + heap safety) |
