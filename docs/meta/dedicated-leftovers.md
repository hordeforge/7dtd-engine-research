...
[platform-auth.md](../admin/platform-auth.md) §authorizers and
[protocol.md](../network/protocol.md) §5 for where that sits in the join sequence;
...
anti-dupe system ([items.md](../gameplay/items.md) §container moves): a lazy singleton
...
`GameManager.gmUpdate`, [loop-gmupdate.md](../loop/loop-gmupdate.md) §10): an
...
uses a shared lock, [loot-economy.md](../gameplay/loot-economy.md)). `ConnectionManager`
...
1. If player already has `singleLocks` / `sharedLocks` / `keepOpenTimes` entry:
   warn + `ForceUnlockByPlayer` then **return** (the IL after the call is `ret`,
   IL_0067), so the new request is refused and nothing is granted; the stale
   state is healed and the player re-requests. The stock client sends
   `UnlockRequestLocal` from `XUiC_LootWindowGroup::OnClose` (IL=37, calls
   `LockManager::UnlockRequestLocal`), so this path is for desync only.
2. Reject null targets; reject span length **> 5** (hard cap). Each reject sets
   `errorMsg` and falls through to the reply, so the client gets a
   `NetPackageLockResponse` with `success=false` (IL_0263), not silence.
...
Complements [items.md](../gameplay/items.md) (bag), [loot-economy.md](../gameplay/loot-economy.md)
(trader lock), [npc-dialog.md](../content/npc-dialog.md) (quest list on trader open).
...
[items.md](../gameplay/items.md); `ItemClassTorch` did not make the cut (see out of scope).
...
[spawning.md](../gameplay/spawning.md)); `PrefabTriggerVolumeList` and
...
[server-browser-prefabs.md](../admin/server-browser-prefabs.md) (prefab load pipeline).
...
[world-generation.md](../world/world-generation.md), which already use these statics.
...
older prefabs. Complements [blocks.md](../world/blocks.md) (current `BlockValue` layout)
and [server-browser-prefabs.md](../admin/server-browser-prefabs.md) (.tts reader).
...
Complements [protocol-packages.md](../network/protocol-packages.md) §2 and
[platform-auth.md](../admin/platform-auth.md) §4.4 (handshake sequencing) and
[network.md](../network/network.md) §4.5 (EncryptStream/DecryptStream layout).
...
dedicated loot spawn path. Complements [loot-economy.md](../gameplay/loot-economy.md).
...
resolution. Complements [platform-auth.md](../admin/platform-auth.md) (platform
abstraction) and [server-lifecycle.md](../admin/server-lifecycle.md) (StartAsServer /
...
[tile-entities-power.md](../gameplay/tile-entities-power.md) and
...
[loop-gmupdate.md](../loop/loop-gmupdate.md) §2 / [managers.md](../loop/managers.md)) and a
...
Complements [spawning.md](../gameplay/spawning.md) and [network.md](../network/network.md).
...
config metadata, with that caveat. Complements [block-shapes.md](../world/block-shapes.md).
...
Complements [blocks.md](../world/blocks.md).
...
[combat-damage.md](../gameplay/combat-damage.md) (hit resolution) and
[entity-ai.md](../entities/entity-ai.md).

## 16. SdFileSystemInfo

The wrapper behind the `SdFile`/`SdDirectory`/`SdFileInfo` facade of
[save-persistence.md](../world/save-persistence.md): holds a `FileSystemInfo` plus the
...
([dedicated-misc-systems.md](../meta/dedicated-misc-systems.md) §TrackedDataMap) as a
...
main loop, [loop.md](../loop/loop.md)). Complements [blocks.md](../world/blocks.md).
...
([signs.md](../gameplay/signs.md)); the listeners themselves are client renderers, so on a
...
Server-side helper of `DynamicMeshManager` ([dynamic-mesh.md](../world/dynamic-mesh.md)
...
route into the pool. Complements [protocol-packages.md](../network/protocol-packages.md)
(package serialization) and [network.md](../network/network.md).
...
prefab/chunk provider path; see [chunk-providers.md](../world/chunk-providers.md) and
[server-browser-prefabs.md](../admin/server-browser-prefabs.md)).

Complements [world-chunks.md](../world/world-chunks.md) (real chunks) by making clear
...
- **ItemClassTorch**: instantiated at `items.xml` parse (`meleeToolTorch`), but
  ...
  block change ([items.md](../gameplay/items.md)).
...
- **`BlockSwitchController` / `BlockSwitchSingleController`**: MonoBehaviours on
  ...
  ([block-shapes.md](../world/block-shapes.md) §triggers).
...
- **ChatTarget**: nested in `XUiC_Chat`; client chat-target dropdown records
  (`Send`, `IsValid`). Server chat routing is in [chat.md](../social/chat.md).
- **GameOptionValue**: nested in `XUiC_GamePrefSelector` and the
  ...
  ([sandbox-options.md](../admin/sandbox-options.md) covers the server side).
...
- **POIBoundsSideHelper**: trigger-collider walls spawned by `POIBoundsHelper`
  ...
  player's client ([quests-challenges.md](../content/quests-challenges.md) §5).
...
- **ChunkPreviewData / ChunkPreviewManager**: prefab-preview visualization
  ...
  [dynamic-mesh.md](../world/dynamic-mesh.md).
...
- **BuffEntityUINotification**: HUD buff notification record. Note it **is**
  ...
  off the server buff tick ([buffs.md](../gameplay/buffs.md)); the allocation is simply inert
...
- **`VariableState*` family** (`VariableStateAbs` base + `VariableStateCVar`,
  ...
  sources are owned by [game-events.md](../content/game-events.md), [entity-stats.md](../entities/entity-stats.md),
  and [sandbox-options.md](../admin/sandbox-options.md).
- **`Archetype`** (10 methods): entity archetype record (the character-preset model
  ...
  [protocol-packages.md](../network/protocol-packages.md) §5.1 (EntityCreationData) and
...
- **`TurretInventory` / `VehicleInventory`**: the specialized inventory holders for
  turrets and vehicles ([vehicles-drones-turrets.md](../gameplay/vehicles-drones-turrets.md)),
  built on the same `Bag`/`ItemStack` shapes as [items.md](../gameplay/items.md).
- **`ThreadRegion`**: the per-thread region work unit of the dynamic-mesh save
  pipeline ([dynamic-mesh.md](../world/dynamic-mesh.md) §4).
- **`PathNodePool`** (`WorldGenerationEngineFinal`): a pooled path-node allocator
  used during RWG road/path routing ([world-generation.md](../world/world-generation.md)).
...
- **`LiveStats`** (18 methods): a survival-stat record (int `liveLevel`,
  ...
  [entity-stats.md](../entities/entity-stats.md)).
...
| Doc | Role |
|---|---|
| [dedicated-misc-systems.md](../meta/dedicated-misc-systems.md) | Sibling grab-bag of small dedicated systems |
| [out-of-scope-surface.md](../meta/out-of-scope-surface.md) | The client/out-of-scope classification boundary |
| [closed-gaps.md](../entities/closed-gaps.md) | Lock-request and other gap closes |
