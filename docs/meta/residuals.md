...
**Pin:** V3.2.0 (b10) dedicated.  
**Coverage of managed families:** [`coverage.md`](../meta/coverage.md).  
...
| DynamicMesh version-160 WriteRegion as live path | [dynamic-mesh.md](../world/dynamic-mesh.md): live `SaveRegion`; `WriteRegion` only self-retry (Xref, 2026-08-06) |
| Region location/timestamp header bit packing | save-region.md §3.5 (Raw + sector) |
| Chunk GetBlock/density index | [terrain-height.md](../world/terrain-height.md), [world-chunks.md](../world/world-chunks.md) (IL dumps in realearth-surfaces-v3.2.0) |
| Chunk write/read layer bound 64 | save-region.md |
| WorldState.SaveLoad structure | save-region.md + dedi-complete §5 |
| Origin.FixedUpdate on dedicated | **No-op:** `IsDedicatedServer` → early `ret` ([loop.md](../loop/loop.md)) |
...
[coverage.md](../meta/coverage.md)). Definition of done: [completion-bar.md](../meta/completion-bar.md)
**tiers A+B**.
...
Partially closed (EAC/EOS wire protocol, native crypto primitives, content
XML, client UI, model limits - all Permanent / out-of-scope / data / model
rows with their closed managed surface stated) plus **optional** annotation
depth (per-flag package framing, per-console-command prose). The two
third-party managed transports (LiteNetLib, A* search internals) were closed
2026-08-12 (network.md §4.6, raycast-pathing.md §5.1). Several previously-open
rows were
...
| Doc | Role |
|---|---|
| [coverage.md](../meta/coverage.md) | Family closed checklist |
| [engine-limitations.md](../meta/engine-limitations.md) | Stock ceilings (including residual-tagged rows) |
| [protocol.md](../network/protocol.md) | Wire residuals vs closed golden packages |
...
- **2026-08-10:** LiteNetLib residual corrected + join-churn flake root cause
  ...
  it -> `Collection was modified` -> `RemoteConnectionClose` ([network.md](../network/network.md)
