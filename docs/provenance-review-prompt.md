...
through this repo's research corpus. Reach 100% coverage, then keep improving
the research itself (new IL pins, live verification, tooling) as the loop.

# Layout (siblings under the workspace root)
- RESEARCH: 7dtd-research/  - stock-engine RE corpus (docs/, tools/, il/)
- ZDTD:     zdtd/          - Zig reimplementation (src/, docs/PROVENANCE.md)
...
# What "provenance" means here (zdtd/docs/PROVENANCE.md)
...
- Changelog: append one dated batch line to RESEARCH workspace/CHANGELOG.md
  per slice (facts, corrections, gate state).
