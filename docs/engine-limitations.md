...
|---|---|---|---|---|
| **Single-thread-dominated sim** | [loop.md](loop.md), [ARCHITECTURE](../../7dtd-server-optimizer/docs/ARCHITECTURE.md) | Extra cores do not parallelize `gmUpdate` / `UpdateTick` | **Hard** | Starve work (LOD, caps); host pin one L3 ([HOST_TUNING](../../7dtd-server-optimizer/docs/HOST_TUNING.md)); do not invent full MT sim in a Harmony mod |
...
| **Registry caps** | stock constants (machine-pinned): `ChunkManager.cMaxChunksSupported` **100000**, `ChunkManager.cMaxChunksAroundPlayers` **15**, `VehicleManager.cMaxVehicles` **500**, `DroneManager.cMaxDrones` **500**, `TurretTracker.cMaxTurrets` **500** (all RE-pinned in `test_tuned_constants.py`); `SleeperVolume` spawn points **255** per volume (inline literal, IL-pinned in [`spawning.md`](spawning.md)) | hard registries: hit the cap and creation fails | **Soft** | stay under the caps; a clone must match or raise them |
...
| [ZIG_CLONE.md](../../zdtd-server-server/docs/ZIG_CLONE.md) | Zig redesign that avoids these walls |
