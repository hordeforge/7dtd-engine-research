...
**Loop context:** [`loop.md`](../loop/loop.md) §10.
...
| `TokenManager` | 121 | |
| `PowerManager` | **106** | Server + players + game started: every **0.16 s** sources then triggers; every **120 s** threaded save; every frame flush `ClientUpdateList` ([tile-entities-power.md](../gameplay/tile-entities-power.md) §3) |
...
| `TriggerManager` | 23 | |
| `EntityAsyncManager` | 22 | Async entity create complete (**phase F**: called after the game-started gate, not the phase-B chain; see [loop-gmupdate.md](../loop/loop-gmupdate.md) §2) |
...
Also from peers / LateUpdate: `MeshDataManager`, `ConnectionManager`, `DynamicMeshManager`, `SdtdConsole`, `LoadManager`, `PlatformManager`, `AstarManager.UpdateGraphs` (185). The last is player-following and the top measured CPU + heap allocator at load: [`measured-scaling.md`](../../../7dtd-server-optimizer/docs/measured-scaling.md) §1/§4b.
...
| Doc | Role |
|---|---|
| [loop.md](../loop/loop.md) | Frame peers |
| [residuals.md](../meta/residuals.md) | ModEvents subscribers residual |

## Changelog

- **2026-08-11:** Manager IL re-verified: DecoManager.UpdateTick IL=330, TwitchManager IL=1585 (exact).
