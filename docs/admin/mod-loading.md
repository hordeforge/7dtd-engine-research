...
**Not:** the `ModEvents` field inventory ([managers.md](../loop/managers.md) owns that);
...
run on shutdown ([server-lifecycle.md](../admin/server-lifecycle.md)).
...
EAC-off ([platform-auth.md](../admin/platform-auth.md)). The real enum values are:
...
owned by [managers.md](../loop/managers.md)**; this doc owns the loading that lets a mod
register them. The game fires these hooks from the corresponding server codepaths
(e.g. `ChatMessage` from [chat.md](../social/chat.md), `PlayerSpawnedInWorld` from
[server-lifecycle.md](../admin/server-lifecycle.md)), so ModEvents is the sanctioned
...
| Doc | Role |
|---|---|
| [managers.md](../loop/managers.md) | `ModEvents` hook field inventory |
| [server-lifecycle.md](../admin/server-lifecycle.md) | Boot (mods load) + shutdown (mods cleanup) |
| [platform-auth.md](../admin/platform-auth.md) | EAC gate that code mods force off |
| [full-surface.md](../meta/full-surface.md) | Whole-assembly map |
| [dedicated-misc-systems.md](../meta/dedicated-misc-systems.md) | Individual *FromXml loaders (entity classes, events, ...) |
| [console-commands.md](../admin/console-commands.md) | `visitmap` host for MapVisitor |
