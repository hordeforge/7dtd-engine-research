...
framing and the join package sequence are in [`protocol.md`](../network/protocol.md) §5, the
pre-auth package set and the public-key encryption handshake are in
[`protocol-packages.md`](../network/protocol-packages.md) §1.4 and §2, and the native
EAC/EOS residuals are already flagged in [`residuals.md`](../meta/residuals.md).
...
[`protocol.md`](../network/protocol.md) §5.
...
[`protocol.md`](../network/protocol.md) section 5 join state machine.
...
1. `InitStreams(true)` -- allocate the full 2 MiB post-auth stream set
   ([network.md](../network/network.md) section 4.1).
...
`AuthConfirmation` body matches [`protocol.md`](../network/protocol.md) §5.
...
the ten `AllowedBeforeAuth` packages ([`protocol-packages.md`](../network/protocol-packages.md)
...
/ C# mod case noted in [`protocol.md`](../network/protocol.md) §1) `EacAuthorizer.AuthorizerActive`
...
drives the public-key handshake described in [`protocol-packages.md`](../network/protocol-packages.md)
...
`NetPackageEncryptionPublicKey` body in [`protocol-packages.md`](../network/protocol-packages.md)
...
- **Runs on dedicated:** the whole `AuthorizationManager` chain runs server-side
  ...
  server `Answer` step in [`protocol.md`](../network/protocol.md) §5.
...
- **Residual (native / external), see [`residuals.md`](../meta/residuals.md):** Steam
  ...
  [`protocol-packages.md`](../network/protocol-packages.md) §2.
...
- **EOS Title Storage cancel path (V3.2.0 b10):**
  `Platform.EOS.RemoteFileStorage` (client-side Title Storage download queue)
  gained a real cancel. Nested `RequestDetails` now carries
  `TitleStorageFileTransferRequest Transfer` and `bool Cancelled` (was
  Filename / LocalUserId / Callback / Chunks only).

```mermaid
flowchart TD
  cancel["CancelGetFile(file, reason)"] --> lock["requestsLock"]
  lock --> find{"TryGetValue"}
  find -->|miss| ret0["return false"]
  find -->|hit| flag["Cancelled = true"]
  flag --> unlock1["release requestsLock"]
  unlock1 --> hasT{"Transfer != null?"}
  hasT -->|yes| eosLock["AntiCheatCommon.LockObject"]
  eosLock --> eosCancel["TitleStorageFileTransferRequest.CancelRequest"]
  eosCancel --> warn{"Result Success or NoChange?"}
  warn -->|no| logWarn["Log.Warning CancelRequest returned"]
  warn -->|yes| complete
  hasT -->|no| complete
  logWarn --> complete["CompleteRequest Other + reason"]
  complete --> cb["ExecuteCallback"]
  queue["TryProcessNextRequest / callbacks"] --> skip{"Cancelled?"}
  skip -->|yes| drop["skip entry"]
  skip -->|no| work["getMetadata / readFile"]
  classDef setup fill:#dbeafe,stroke:#3b82f6,color:#1e3a5f
  classDef sync fill:#dcfce7,stroke:#22c55e,color:#14532d
  classDef danger fill:#fee2e2,stroke:#ef4444,color:#7f1d1d
  class cancel,lock,find,flag setup
  class complete,cb,work sync
  class ret0,drop,logWarn danger
```

  `CancelGetFile` (IL 16→70) sets `Cancelled` under `requestsLock`, calls
  `TitleStorageFileTransferRequest.CancelRequest()` under
  `AntiCheatCommon.LockObject` when a transfer handle exists, then
  `CompleteRequest` with `EFileDownloadResult.Other` (`ldc.i4.3`; Ok=0,
  EmptyFilename=1, FileNotFound=2, Other=3) and the cancel reason.
  `TryProcessNextRequest` / `queryFileCallback` / `readCompletedCallback` skip
  cancelled entries; `readFile` stores the EOS transfer handle; `CompleteRequest`
  ignores completions for requests no longer tracked (`TryGetValue` + reference
  eq; `Log.Out` on miss). Local `Platform.Local.RemoteFileStorage.CancelGetFile`
  remains `NotImplementedException`. No wire / NetPackage / join-auth impact.
  Full IL table: [changelog-3.2.0.md](../releases/changelog-3.2.0.md) §8.
...
| Doc | Role |
|---|---|
| [protocol.md](../network/protocol.md) | Wire framing + the join package sequence this feeds (§5) |
| [protocol-packages.md](../network/protocol-packages.md) | Pre-auth package set (§1.4) + the encryption handshake bodies (§2) |
| [protocol-frames.md](../network/protocol-frames.md) | Visual byte frames for the login / auth packages |
| [network.md](../network/network.md) | Where auth sits in the connection lifecycle |
| [server-lifecycle.md](../admin/server-lifecycle.md) | Server start / stop that calls `ServerStart` on the authorizers |
| [managers.md](../loop/managers.md) | Other in-process managers (`ConnectionManager`, `GameManager`) |
| [residuals.md](../meta/residuals.md) | Native EAC / EOS / Steam residuals |
| [re-methodology.md](../meta/re-methodology.md) | How this was reversed from IL |
| [INDEX.md](../INDEX.md) | Hub |

## Changelog

- **2026-09-05:** V3.2.0 b10: EOS `RemoteFileStorage` cancel path (`RequestDetails.Transfer`/`Cancelled`, `CancelGetFile` IL 16→70, completes with `EFileDownloadResult.Other`). Flow diagram in §6; IL table in [changelog-3.2.0.md](../releases/changelog-3.2.0.md) §8.
