...
damage / upgrade / tick lifecycle (owned by [`blocks.md`](../world/blocks.md)); the chunk
mesh/light/water pipeline that actually consumes `renderFull`/`renderFace`
(owned by [`light-mesh-water.md`](../world/light-mesh-water.md)); sleeper volume ticking
(owned by [`spawning.md`](../gameplay/spawning.md)); wire framing (owned by
[`protocol-packages.md`](../network/protocol-packages.md)); block XML content and client
...
described in [`blocks.md`](../world/blocks.md).
...
|---|---|
| How do the `rawData` rotation bits (16..20, see [`blocks.md`](../world/blocks.md)) map to an orientation, and what does "rotate" mean? | `GetRotation`, `RotateY`, `Rotate`, `MirrorY`, `GetRotatedBlockFace` |
| What physical extent does the cell have? | `GetBounds`, `GetStepHeight`, `IsMovementBlocked` |
| What mesh does the cell emit? | `renderFull`, `renderFace`, `renderDecorations`, `getFacesDrawnFullBitfield`, `isRenderFace` (consumed by the mesh generators in [`light-mesh-water.md`](../world/light-mesh-water.md); `MeshPurpose` = `World/Drop/Hold/Local/Preview/SimplifiedCollisionOnly`) |
...
[`light-mesh-water.md`](../world/light-mesh-water.md)), which is why every shape also
...
| `BlockGameEvent` | fires its `onTriggeredEvent` through `GameEventManager` (target type must be `Block`, see [`game-events.md`](../content/game-events.md)) |
...
- **Deferred fire:** triggers saved with `NeedsTriggered=1` (used by the quest
  ...
  `GameManager.gmUpdate`, see [`loop-gmupdate.md`](../loop/loop-gmupdate.md)) counts it
...
- **Scripted:** `MinScript.Tick` can pulse a raw channel via
  ...
  [`minevents.md`](../content/minevents.md)); the GameEvent actions named
  `ActionBlockTriggerFall` / `ActionBlockTriggerMines` are unrelated to this
  system despite the name (they belong to [`game-events.md`](../content/game-events.md)).
...
| Doc | Role |
|---|---|
| [`blocks.md`](../world/blocks.md) | `BlockValue` word (rotation bits), `Block` flyweight and lifecycle hooks that forward to the shape |
| [`light-mesh-water.md`](../world/light-mesh-water.md) | The mesh/water pipeline that consumes `renderFull`/`renderFace` and the flow masks |
| [`world-chunks.md`](../world/world-chunks.md) | Chunk storage the trigger dictionary and entity block stubs live in |
| [`save-region.md`](../world/save-region.md) | Chunk read/write that persists `BlockTrigger` records |
| [`spawning.md`](../gameplay/spawning.md) | Sleeper volumes that listen on trigger channels |
| [`quests-challenges.md`](../content/quests-challenges.md) | Quest reset flow that drives `RefreshTriggers` / `NeedsTriggered` |
| [`game-events.md`](../content/game-events.md) | `BlockGameEvent.OnTriggered` target; the unrelated `ActionBlockTrigger*` actions |
| [`loop-gmupdate.md`](../loop/loop-gmupdate.md) | Ticks `TriggerManager.Update` |
| [`protocol-packages.md`](../network/protocol-packages.md) | `NetPackageBlockTrigger` framing (ToServer only) |
