...
engine path (owned by [`world-chunks.md`](../world/world-chunks.md)); chunk and region
persistence (owned by [`save-region.md`](../world/save-region.md)); the `TileEntity` heap
objects and the power graph that back workstations, loot, and traps (owned by
[`tile-entities-power.md`](../gameplay/tile-entities-power.md)); the on-wire
`BlockChangeInfo` framing (owned by [`protocol-packages.md`](../network/protocol-packages.md));
...
([`tile-entities-power.md`](../gameplay/tile-entities-power.md)); everything else lives in the
word.
...
pipeline, [items.md](../gameplay/items.md) §2). Concrete pins from the V3.2.0 cctor:
...
1. **Engine mutation.** `ChunkCluster.SetBlock` (IL=828, owned by
   [`world-chunks.md`](../world/world-chunks.md)) writes the new `BlockValue` into the
...
3. **Wire.** The queued change is serialized as a `BlockChangeInfo` inside
   `NetPackageSetBlock` (see [`protocol-packages.md`](../network/protocol-packages.md) 6.1):
...
([`world-chunks.md`](../world/world-chunks.md)); a `PropReference` ref goes through
...
`-1` / `0.1` s (see [`protocol-packages.md`](../network/protocol-packages.md) §6.14).
...
4. Else scan chunks in claim half-extent (`GameStats` **44**, same ring math as
   ...
   deny when `forKeystone=false` (see [server-lifecycle.md](../admin/server-lifecycle.md)
   §3.1).
...
`EntityPlayerLocal` (the bedroll becomes the spawn point, [spawning.md](../gameplay/spawning.md)) and, for
...
loop (`Chunk.UpdateTick`, [`tile-entities-power.md`](../gameplay/tile-entities-power.md)) walks
heap objects; the **block** ticker (`WorldBlockTicker`, driven from
`World.OnUpdateTick`, [`world-chunks.md`](../world/world-chunks.md) §6) calls
...
| Powered | `BlockPowered`, `BlockPoweredDoor/Light/Trap`, `BlockPowerSource`, `BlockSwitch` | `CreateTileEntity`, `GetBlockActivationCommands`, `OnBlockActivated`, `OnBlockLoaded` (relink) | create a `TileEntityPowered`, expose wire/activation commands (see [`tile-entities-power.md`](../gameplay/tile-entities-power.md)) |
...
| Containers / doors / signs | (no standalone `Block` class) | expressed as `TileEntityComposite` features (`TEFeatureDoor`, loot, sign) | loot, secure loot, land claim, and doors are **composite tile-entity features**, not dedicated block classes ([`tile-entities-power.md`](../gameplay/tile-entities-power.md) 1.1) |
...
(the powered-ranged-trap base, [tile-entities-power.md](../gameplay/tile-entities-power.md) §3.7);
...
- **Two-phase change.** The engine `SetBlock` path (chunk storage, dirty flags)
  is [`world-chunks.md`](../world/world-chunks.md); the block callbacks and the RPC helper
...
| Doc | Role |
|---|---|
| [world-chunks.md](../world/world-chunks.md) | Chunk block storage arrays and the `ChunkCluster.SetBlock` engine mutator |
| [save-region.md](../world/save-region.md) | Where `BlockValue` words are written inside the chunk blob and region files |
| [tile-entities-power.md](../gameplay/tile-entities-power.md) | The `TileEntity` heap objects and power graph that back powered, loot, and composite blocks |
| [protocol-packages.md](../network/protocol-packages.md) | `NetPackageSetBlock` and the `BlockChangeInfo` wire layout |
...
- **2026-08-11:** Placement IL re-verified: Plate/Spotlight/TowardsPlacer90/TowardsPlacerInverted.OnPlaceBlock IL=91, PineLeaves IL=26, Torch IL=53, DrawBridge.LimitRotation IL=35, SleepingBag rotationToAddVector IL=25, CanPlaceBlockAt IL=80, PlaceBlock IL=52, GetOwningPlayer IL=43, OnBlockActivated IL=16, TakeItemWithTimerDone IL=106 (exact).
- **2026-08-11:** Tick/behavior IL re-verified: BlockPlantGrowing.UpdateTick IL=239 / addScheduledTick IL=63, TorchHeatMap.UpdateTick IL=35, OnEntityWalking IL=1 (JumpPad IL=5, Mine IL=113), Cactus.GetCollisionAABB IL=60, Hay IL=34 / OnEntityCollidedWithBlock IL=10, Ladder.IsElevator IL=2/6, Stairs.IsMovementBlocked IL=7, Deadgrass.Init IL=6 / OnBlockPlaceBefore IL=8, TrunkTip.RotateVerticesOnCollisionCheck IL=2, SiblingRemove.Init IL=20 / OnBlockRemoved IL=177, Music OnBlockAdded/OnBlockRemoved IL=1, Ranged.Init IL=16, Forge checkParticles IL=15 / MaterialUpdate IL=64 / GetActivationText IL=29, Sign.Init IL=27 / OnBlockAdded IL=50 (exact).
