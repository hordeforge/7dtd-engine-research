# Save/persistence orchestration: managed paths, slots, save info (dedicated V3.2.0)
...
([save-region.md](../world/save-region.md)); when saves are triggered during boot/shutdown
([server-lifecycle.md](../admin/server-lifecycle.md)); player-file contents
([server-lifecycle.md](../admin/server-lifecycle.md) §3).
...
`GameManager.StartGame`; see [server-lifecycle.md](../admin/server-lifecycle.md) §1) yields
...
- **Size tracking:** `InitFileSizeTracking` (IL=70) folds every file into
  ...
  `Region`, i.e. region files ([save-region.md](../world/save-region.md) §3) are the priority
...
- **`ProcessSaveEntry`** (IL=182): reads the `main.ttw` **WorldState header**
  ([save-region.md](../world/save-region.md) §1) to get `gameVersion` and `saveDataLimit`;
...
| Doc | Why |
|---|---|
| [save-region.md](../world/save-region.md) | WorldState/`main.ttw` header this layer reads; region files this layer prioritizes |
| [server-lifecycle.md](../admin/server-lifecycle.md) | boot/shutdown points where init, save, and cleanup fire |
| [world-chunks.md](../world/world-chunks.md) | RegionFileManager that registers with the manager |
| [managers.md](../loop/managers.md) | where these singletons sit among the other managers |
| [console-commands.md](../admin/console-commands.md) | `sdminfo` diagnostic command |
