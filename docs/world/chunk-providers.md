...
trampoline internals ([`world-chunks.md`](../world/world-chunks.md)); `RegionFileManager`
on-disk layout ([`save-region.md`](../world/save-region.md)); RWG world creation
([`world-generation.md`](../world/world-generation.md)); `DecoManager.UpdateTick` frame
cost ([`loop.md`](../loop/loop.md), [`loop-gmupdate.md`](../loop/loop-gmupdate.md)).
...
provider-agnostic and lives in [`world-chunks.md`](../world/world-chunks.md).
...
([`world-generation.md`](../world/world-generation.md)); this provider then *consumes*
...
written ([`save-region.md`](../world/save-region.md)).
...
1. Under `lockObject`, copy `m_AllChunkPositions.list` into
   ...
   from `DetermineChunksToLoad`, [world-chunks.md](../world/world-chunks.md) section 4.0.1).
...
([dynamic-mesh.md](../world/dynamic-mesh.md) `SetNextChunkToLoad`).
...
3. Otherwise allocate from `MemoryPools.PoolChunks`, set X/Z, derive a
   ...
   [`world-chunks.md`](../world/world-chunks.md) §3, [`terrain-height.md`](../world/terrain-height.md)).
...
- `SaveAll()`: editor saves prefab decorator + spawn points; game mode
  ...
  ([`save-region.md`](../world/save-region.md)).
...
[`protocol-packages.md`](../network/protocol-packages.md) §4.2 `fixedSizeCC`.
...
- `DecoObject` is `{Vector3i pos, float realYPos, BlockValue bv, DecoState
  ...
  **`bv.rawData:u32`**, **`state:u8`**, after which the block is registered in the
  ...
  `DecoManager.Read(br, version, resetExisting)` (IL=29) mirrors the write:
  it (re)creates the `loadedDecos` set when `resetExisting`, reads the `int32`
  count, and `loadedDecos.Add`s one `DecoObject.Read` per entry.
...
- Model side: `DecoChunk.AddDecoObject(_tryInstantiate)` calls
  ...
  `DecoManager.UpdateTick` cost on dedi is covered in [`loop.md`](../loop/loop.md).
...
- **`ChunkCacheNeighborBlocks`** (with `ChunkCacheNeighborChunks`): a cached
  ...
  `DynamicMeshChunkProcessor.Init` ([`dynamic-mesh.md`](../world/dynamic-mesh.md)), so
...
- **`ChunkBlockLayerLegacy`**: the pre-`ChunkBlockChannel` 4-block-tall layer
  ...
  `MeshGenerator*` and `Prefab.PrefabChunk` ([`world-chunks.md`](../world/world-chunks.md) §2).
...
| Doc | Relationship |
|---|---|
| [`world-chunks.md`](../world/world-chunks.md) | Chunk cache, tick, load/send, SetBlock; consumes what providers produce |
| [`world-generation.md`](../world/world-generation.md) | RWG creates the world files this layer reads at runtime |
| [`save-region.md`](../world/save-region.md) | `RegionFileManager` layout, WorldState/`main.ttw`, save chain |
| [`terrain-height.md`](../world/terrain-height.md) | `ITerrainGenerator` height math behind `generateTerrain` |
| [`dynamic-mesh.md`](../world/dynamic-mesh.md) | `DynamicMeshThread` feeds the GenerateChunks thread; neighbor block cache user |
| [`loop.md`](../loop/loop.md) / [`loop-gmupdate.md`](../loop/loop-gmupdate.md) | Frame cost of `provider.Update`, `DecoManager.UpdateTick`, `SaveRandomChunks` |
| [`light-mesh-water.md`](../world/light-mesh-water.md) | Lighting/water stages that follow decoration |
...
- **2026-08-11:** Generation IL re-verified: GetWorldCreationData IL=3, ChunkManager.GetNextChunkToProvide IL=102, World.GetNextChunkToProvide IL=4, GenerateChunksThread IL=36, DynamicMeshThread.GetNextChunkToLoad IL=18, GenerateSingleChunk IL=171 (exact).
- **2026-08-11:** Decorator IL re-verified: DecorateChunkOverlapping IL=245 (BlocksFromBiome) / IL=472 (POIFromImage), GetSubBiomeIdxAt IL=79, GetBiomeOrSubAt IL=24, decorateSingleBlocks IL=56, decorateSingleBlock IL=139, decoratePrefabs IL=403, Prefab.SnapTerrainToArea IL=65, ChunkCluster.snapTerrainToPosition IL=113 / SnapTerrainToPositionAroundRPC IL=49, PoiMapElement GetRandomBlockOnTop/GetRandomDecal IL=26, GetDecal IL=15 (exact).
- **2026-08-11:** Water/grid IL re-verified: LoadWaterInfo IL=127, GetWaterChunks16x16 IL=9, InitData IL=6, <InitData>d__15 MoveNext IL=774, WorldGridCompressedData.Contains IL=25 / GetData IL=18, GameUtils.WaterFloodFill IL=196, WorldBlockFiller resetBlockInformation IL=41, setBlockToFill IL=81, fillChunk IL=66, fillLevel IL=192, setDecorationBlock IL=152 (exact).
- **2026-08-11:** DTM-processing IL re-verified: getFilenameDTM IL=17, loadDTM IL=25, ctor IL=28, calcWorldFileCrcs IL=9 + MoveNext IL=121, filesNeedProcessing IL=32, verifyFileHashes IL=61, processFiles IL=12 + MoveNext IL=918 + b__0 IL=13 (exact).
