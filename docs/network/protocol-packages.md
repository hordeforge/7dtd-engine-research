...
**Method:** [`re-methodology.md`](../meta/re-methodology.md) §4-5.
...
[`protocol.md`](../network/protocol.md) §8 ("bots use uncompressed path"): true for the
...
([`save-region.md`](../world/save-region.md) §2): stability channel skipped, custom-data
...
([world-chunks.md](../world/world-chunks.md) section 4.0).
...
against the [zdtd](../../../zdtd-server/docs/) clone's zombie spawn, which correctly writes the
empty middle.)
...
Selection thresholds live in [network.md](../network/network.md) section 2 (encoded
...
[save-region.md](../world/save-region.md) section 1.3; this package is the C2S transport
...
Jobified water sim stream ([light-mesh-water.md](../world/light-mesh-water.md) section 4).
...
order also in [protocol.md](../network/protocol.md) section 6.5; V3.2.0 b9 `write` IL=144
(V3.1.0 was IL=176).
...
`flags` bit layout, from the `cFlags*` const fields on `NetPackageDamageEntity`
(each bit replaces the like-named V3.1.0 bool; `bIsDamageTransfer` was dropped
with no replacement bit):

| Bit | Mask | Value | Const | V3.1.0 bool |
|---:|---|---:|---|---|
| 0 | 0x001 | 1 | `cFlagsCanHitSpecialBodyParts` | `bCanHitSpecialBodyParts` |
| 1 | 0x002 | 2 | `cFlagsCrippleLegs` | `bCrippleLegs` |
| 2 | 0x004 | 4 | `cFlagsCritical` | `bCritical` |
| 3 | 0x008 | 8 | `cFlagsDismember` | `bDismember` |
| 4 | 0x010 | 16 | `cFlagsFatal` | `bFatal` |
| 5 | 0x020 | 32 | `cFlagsFromBuff` | `bFromBuff` |
| 6 | 0x040 | 64 | `cFlagsIgnoreConsecutiveDamages` | `bIgnoreConsecutiveDamages` |
| 7 | 0x080 | 128 | `cFlagsIgnorePartyShare` | `bIgnorePartyShare` |
| 8 | 0x100 | 256 | `cFlagsPainHit` | `bPainHit` |
| 9 | 0x200 | 512 | `cFlagsTurnIntoCrawler` | `bTurnIntoCrawler` |
| 10 | 0x400 | 1024 | `cFlagsTrapKillXP` | new in V3.2.0 |
...
Apply path owned by [combat-damage.md](../gameplay/combat-damage.md).
...
[tile-entities-power.md](../gameplay/tile-entities-power.md) section 2.

### 6.13 Inventory transaction packages

Server-authoritative container moves ([items.md](../gameplay/items.md)).
...
Also summarized in [protocol.md](../network/protocol.md) section 6.6.
...
Routing is recipient-list based ([chat.md](../social/chat.md)).

#### `NetPackageConsoleCmdServer` / `Client`

See [console-commands.md](../admin/console-commands.md) section 3: server carries a single
`cmd` string; client carries `lineCount` strings + `bExecute`.

#### `NetPackageLockRequest` / `Response`

See [dedicated-leftovers.md](../meta/dedicated-leftovers.md) section 2.2: locking bool,
channel u16, target identifying infos, context type name + body.

#### `NetPackagePartyActions` / `PartyData`

Already in [parties-factions.md](../social/parties-factions.md) section 3. Re-verified:
...
`tePosition` and `CopyFrom` + `NotifyListeners` ([loot-economy.md](../gameplay/loot-economy.md)).
...
quest state (full case table: [quests-challenges.md](../content/quests-challenges.md) §8).
...
Detail: [quests-challenges.md](../content/quests-challenges.md) section 8,
[npc-dialog.md](../content/npc-dialog.md).
...
Already in [parties-factions.md](../social/parties-factions.md) section 5.2 (platform ids +
status/events).
...
`IsRepairing=false`. Detail: [server-lifecycle.md](../admin/server-lifecycle.md) section 6.
...
Full 0..16 table: [quests-challenges.md](../content/quests-challenges.md) section 8.
...
[weather-environment.md](../gameplay/weather-environment.md) section 3.
...
([map-objects.md](../gameplay/map-objects.md)).
...
([signs.md](../gameplay/signs.md) section 4). Response is compressed.
...
Process path already in [protocol.md](../network/protocol.md) section 5 / RequestToSpawnPlayer narrative.
...
server. Detail: [dynamic-mesh.md](../world/dynamic-mesh.md).
...
shows the dropped-backpack markers. zdtd tracks three markers per player
(`max_tracked_backpacks`, oldest evicted like stock), broadcast on drop,
cleared on collect, persisted in the ZPV13 player record and re-sent in the
join bundle so a marker restored from disk reaches the clients.
...
trio: [protocol-packages.md §3.3](../network/protocol-packages.md) MapChunks (terrain)
...
| Bulk-package compression codec | **Closed (2026-08-12):** the "LZ/native" reading was wrong - `NetConnectionSteam` fields + `NetConnectionAbs.Compress` (IL=59) / `Decompress` (IL=22) run the **managed `Noemax.GZip.DeflateOutputStream`** (level 3, raw deflate, no header) over the package stream, the same codec as the region payload ([save-region.md](../world/save-region.md) §3.4, byte-exact-verified). No native codec |
...
| Doc | Role |
|---|---|
| [protocol.md](../network/protocol.md) | Framing, challenge, join, golden motion bodies |
| [protocol-frames.md](../network/protocol-frames.md) | Visual byte frames |
| [re-methodology.md](../meta/re-methodology.md) | How these were derived |
| [network.md](../network/network.md) | Interest / replication / bands |
...
- **2026-08-10:** NetPackageMinEventFire stock defect documented (§6.23):
  ...
  churn (108 in one 28-bot run). Cross-refs [network.md](../network/network.md) §4.0.
