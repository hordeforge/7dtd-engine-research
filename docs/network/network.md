...
**Current pin:** V **3.1.0 (b14)**. Managed net architecture was first RE'd on V3.0.1 and re-checked on the live 3.1.0 assembly. V3.1 wire/join deltas live in topic docs: TE outer wire in [protocol-packages.md](../network/protocol-packages.md) §6.12; PackageIds version minor=10 build=14 in [protocol.md](../network/protocol.md) / loadgen fixtures; GSI / sandbox browser fields in [server-browser-prefabs.md](../admin/server-browser-prefabs.md). Delta map: [changelog-3.1.0.md](../releases/changelog-3.1.0.md) § Per-fact homes. **Hub:** [`INDEX.md`](../INDEX.md).
...
**Wire framing / join / golden bodies:** [`protocol.md`](../network/protocol.md).  
**Visual frames:** [`protocol-frames.md`](../network/protocol-frames.md).  
**Companion:** [`closed-gaps.md`](../entities/closed-gaps.md) §4 (threshold decode).  
**Ceiling map:** [`engine-limitations.md`](../meta/engine-limitations.md) §2-3 (player O(N²), packages).  
**Clone design:** [`ZIG_CLONE.md`](../../../zdtd-server/docs/ZIG_CLONE.md).  
**Loop:** [`loop.md`](../loop/loop.md) §6.  
...
```mermaid
flowchart TD
  CU[ConnectionManager.Update IL=231]
...
These run as **peer MonoBehaviours** relative to `GameManager.Update`. **Observed order (2026-08-09, runtime probe):** `ConnectionManager.Update` runs **before** `GameManager.Update` every frame (and `ConnectionManager.LateUpdate` before `GameManager.LateUpdate`); see [loop.md](../loop/loop.md) §1.1.
...
### 1.1 `ConnectionManager.Update` (IL=231, server path)
...
2. For each package: reject wrong `PackageDirection` (warn); reject
   ...
   see [dedicated-leftovers.md](../meta/dedicated-leftovers.md) §12).
...
Detail: [world-chunks.md](../world/world-chunks.md) section 4.0.
...
Wire bodies for these packages: [protocol-packages.md](../network/protocol-packages.md)
...
Detail on spawn body: [protocol-packages.md](../network/protocol-packages.md) section 5.1.
...
Live census (tools/bin/Census.exe): **196** types with the `NetPackage*` name prefix = **195 + `NetPackageManager`**. Of those 195, the **191** in the live id-map are the actual registered wire packages; the remaining **4** are the abstract base `NetPackage`, the abstract intermediate `NetPackageEntityTargeted`, the abstract helper `NetPackageLogger`, and the `NetPackageDirection` enum (195 - 4 = 191, derived from the V3.2.0 b10 census; the last live join capture was 189 on V3.1.0 b14, see §5). `NetPackageMetrics` is a concrete class and *is* registered.
...
Join + envelope + golden entity package bodies: [protocol.md](../network/protocol.md).
...
Port: LiteNet often **ServerPort+2** (26902). Details and binary layouts: [protocol.md](../network/protocol.md).
...
| `AesEncryptAndMac` (`IEncryptionModule`) | Yes (AES + HMACSHA256) | stream layout below; handshake packages in [protocol-packages.md](../network/protocol-packages.md) §2 |
...
`ConnectionManager.Update` (IL=231, peer MB) drains each connection via `GetPackages` → `ProcessPackages` (IL=116) and flushes send queues. Xref: `ProcessPackages` is only called from that Update path (4 sites).
...
Implements `IEncryptionModule` for the post-handshake session (constructed in `AntiCheatEncryptionAuthServer.SendSharedKey` with the two key blobs from `NetPackageEncryptionSharedKey`; see [dedicated-leftovers.md](../meta/dedicated-leftovers.md) §7 and [protocol-packages.md](../network/protocol-packages.md) §2).
...
[residuals.md](../meta/residuals.md): the transport algorithms the dedicated server
actually runs.
...
| Doc | Why |
|---|---|
| [loop.md](../loop/loop.md) | UpdateTick placement of replication; frame peers |
| [closed-gaps.md](../entities/closed-gaps.md) | Distance-band threshold decode evidence |
| [entity-ai.md](../entities/entity-ai.md) | What is being replicated |
| [measured-scaling.md](../../../7dtd-server-optimizer/docs/measured-scaling.md) | Super-linear player-axis cost (optimizer) |
| [protocol-packages.md](../network/protocol-packages.md) | Encryption handshake package bodies |
| [dedicated-leftovers.md](../meta/dedicated-leftovers.md) | AesEncryptAndMac install from SendSharedKey |
...
- **2026-09-11:** IL citations refreshed against the V3.2.0 b10 assembly: `ConnectionManager.Update` 228 -> 231 (exact). Prior values were the V3.1.0 b14 measurement.
...
- **2026-08-11:** ConnectionManager IL re-verified: Update IL=231, LateUpdate IL=4, ProcessPackages IL=116, DisconnectClient IL=184, SavePlayerData IL=91 (GameManager), SendPackage(list) IL=168, AddClient IL=32, ServerReady IL=10, SendToClientsOrServer IL=21, Enable/Disable/PrintNetworkStatistics IL=4, SetLatencySimulation IL=7, SetPacketLossSimulation IL=6, get_CurrentMode/get_IsClient/GetRequiredPortsString IL=4, Net_DataReceivedServer IL=14, Net_PlayerConnected IL=12, SendLogin IL=60, Net_InvalidPassword IL=16, Net_ConnectionFailed IL=30, Net_DisconnectedFromServer IL=22 (exact).
